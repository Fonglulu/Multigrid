...
The test currently builds a uniform grid of a given size, and stores the
5-point finite difference stencil in the connection table. It then extracts
the matrix from the connection table uses the input python direct solver
to find the solution to a model test problem. The two model test problems are
current, u = sin(pi x)*sin(pi y) and u = exp(x)*exp(y)
